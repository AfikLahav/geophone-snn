...
def cultural_lines(n, rng, fs, n_extra=None):
    ...
    # v4.1 (GEO_MACH_P): real rigs showed NO such lines (DISCREPANCY_REPORT sp_lines) -> make
    # presence probabilistic (P) with 1-2 lines when present. Default = v4 behavior.
    _mp = os.environ.get("GEO_MACH_P")
    if n_extra is not None:
        n_mach = n_extra
    elif _mp is not None:
        n_mach = int(rng.integers(1, 3)) if rng.random() < float(_mp) else 0
    else:
        n_mach = rng.integers(0, 4)
...
def traffic_hum(n, rng, fs=1000.0):
    """v4 confuser: distant-highway seismic hum. Continuous colored ground velocity peaking
    <25 Hz (no impulsive structure) -> a 'fools-vehicle' non-target on road-adjacent sites.
    Returns ground velocity (m/s). (Riahi & Gerstoft 2015 traffic footprint.)"""
    f = np.fft.rfftfreq(n, 1 / fs)
    shape = np.zeros_like(f)
    m = (f >= 2.0) & (f <= 25.0)
    shape[m] = (np.maximum(f[m], 2.0)) ** (-0.5)                 # power ~ f^-1
    spec = shape * (rng.standard_normal(len(f)) + 1j * rng.standard_normal(len(f)))
    x = np.fft.irfft(spec, n=n); x /= (x.std() + 1e-30)
    lvl = 10 ** rng.uniform(-7.2, -6.2)                          # m/s RMS, distant-road scale
    return (x * lvl).astype(np.float64)
