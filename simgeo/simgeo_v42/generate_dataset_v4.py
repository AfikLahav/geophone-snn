"""Generate the v4.3.1 synthetic geophone dataset.

The generator samples target SNR, terrain, weather, source type, distance, and
sensor-chain variation. It stores source-only and background components so labels
can be derived without changing the model input.

Usage:
    python simgeo/simgeo_v42/generate_dataset_v4.py [n_scenes] [out_dir] [shards] [max_scenes]

The default full run contains 171,700 scenes and writes to
``$GEO_SYNTH_ROOT/dataset_v431``. Use an output directory whose basename starts
with ``dataset_v431_pilot`` for a limited verification run.
"""
...
import generate_dataset as gc                         # unchanged constants/helpers (MIN_STANDOFF, LINE_TIER, ...)
...
from config_paths import CFG_DIR as _CFG_DIR
SNRMAP_PATH = os.path.join(_CFG_DIR, "snr_maps_v431.npz")
SUBKIND_CEILINGS_PATH = os.path.join(_CFG_DIR, "subkind_ceilings_v431.json")
...
def _emit_moving(c, csub, cond, has_gaps, rng, pid, bank, force_snr=None):
    ...
    # v4.3.1: position re-indexing by the speed profile is HUMAN-only (cadence follows
    # v_profile there); vehicle/animal keep constant nominal path speed.
    ...
    ems = []; infos = []; ns = 1; is_animal = False
    if c == "human":
        if csub in ("group", "march"):
            ...
            ems, info = src.human(path, tg, rng, n_bank, "walk", payload_kg=pay,
                                  cadence_mul=rng.uniform(1.0, 1.1), v_profile=pi.get("v_profile"))
            ...
            ems, info = src.human(path + off, tg, rng, n_bank, gait,
                                  v_profile=pi.get("v_profile")); infos.append(info)
    elif c == "vehicle":
        if csub == "convoy":
        ...
        if csub == "herd":
            ...
            leader_kind = str(rng.choice(gc.HERD_SPECIES))
            leader_wav = leader_kind if leader_kind in ("dog", "jackal", "boar", "horse") else "boar"
            # v4.3: shared schedule for group sync (08_ANIMAL_BEHAVIOR_RESEARCH.md §7.4)
            leader_sched = src._activity_schedule(rng, dur, leader_wav)
            for mi in range(ns):
                species = str(rng.choice(gc.HERD_SPECIES)) if mi > 0 else leader_kind
                wav = species if species in ("dog", "jackal", "boar", "horse") else "boar"
                off = rng.uniform(0, 2, 2)
                # shift schedule boundaries by a small per-member lag
                dt_lag = float(rng.uniform(-1.5, 1.5))
                shifted = [(max(0, t0 + dt_lag), min(dur, t1 + dt_lag), st, g)
                           for t0, t1, st, g in leader_sched if t1 + dt_lag > 0]
                em, info = src.animal(path + off, tg, rng, n_bank, wav, schedule=shifted)
    ...
    # v4.3: animals use their own bout schedule for pauses, not _apply_gaps
    if is_animal:
        ivs = _merge([(e[0] - 0.05, e[0] + 0.45) for e in ems]) if ems else []
    else:
...
def gen_scene_v42(spec):
    ...
    fc = fc_native                                                # v4.3: coupling removed (passthrough)
    ...
    # v4.3: H matches render_hp — geophone only, no coupling/spurious.
    H = sensor.geophone_response(n, p["f0"], p["h"], p["G"])
    cbc = {}
...
def init_db_v42(path):
    ...
    # dataset_v431/v4b/v41 -> and this fn DELETES path/-wal/-shm below). Refuse every output outside the v4.3.1 naming rule.
    base = os.path.basename(os.path.dirname(path))
    assert base in ("dataset_v431",) or base.startswith(("dataset_v431_pilot",)), \
        f"refusing unrecognized path (data-loss guard): {path}"
    for sfx in ("", "-wal", "-shm"):
...
def run_shard_v42(args):
    ...
    if committed:
        print(f"  v4.3.1-shard{wi}: RESUME {len(committed)} done, {len(todo)} to go", flush=True)
    ...
    for spec in todo:
        try:
            ...
            fail += 1
            if fail <= 5:
                print(f"  v4.3.1-shard{wi} scene {spec[0]} FAIL {type(e).__name__}: {e}", flush=True)
        if len(sbuf) >= 200:
            ...
            db.commit(); sbuf, vbuf = [], []
            db.execute("PRAGMA wal_checkpoint(TRUNCATE)")
            print(f"  v4.3.1-shard{wi}: {done} scenes, {(time.time()-t0)/60:.1f} min", flush=True)
    if sbuf:
        db.executemany(f"INSERT INTO scenes({SCENE_COLS_V42}) VALUES ({','.join('?'*ns)})", sbuf)
        db.executemany(f"INSERT INTO waveforms({WAVE_COLS_V42}) VALUES ({','.join('?'*nv)})", vbuf)
        db.commit()
...
def main():
    n_target = int(sys.argv[1]) if len(sys.argv) > 1 else 171700
    out_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(_CFG_DIR, "..", "dataset_v431")
    ...
    assert _base in ("dataset_v431",) or _base.startswith(("dataset_v431_pilot",)), \
        f"refusing unrecognized out_dir (data-loss guard): {out_dir}"
    ...
    print(f"v4.3.1 plan: {len(plan)} scenes ({ncells} cells; {comp}), {nw} shards / {pool} workers -> {out_dir}", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=pool) as ex:
        for wi, done, fail in ex.map(run_shard_v42, args):
            print(f"v4.3.1-shard {wi}: {done} scenes, {fail} failed", flush=True)
    dt = (time.time() - t0) / 60
