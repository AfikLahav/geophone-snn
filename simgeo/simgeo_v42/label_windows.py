...
# total windows/subkind equalize. Kept OFF by default; populated from the pilot duration audit.
KCAP_PATH = os.environ.get("GEO_V42_KCAP")
KCAP = json.load(open(KCAP_PATH)) if (KCAP_PATH and os.path.exists(KCAP_PATH)) else None
...
def run_shard(args):
    ...
    for sid, n, nmv, cm1, cls1, cm2, cls2 in cur:
        try:
            ...
            # window positions (even-stride quota). v4.2 Change 4: per-subkind cap when GEO_V42_KCAP
            # is set (equalizes windows/subkind so slow subkinds stop over-contributing); else uniform.
            Kc = K if KCAP is None else min(K, int(KCAP.get(m[3], K)))
            pos = list(range(0, max(1, n - nw + 1), nh))
            if len(pos) > Kc:
                pos = [pos[j] for j in np.round(np.linspace(0, len(pos) - 1, Kc)).astype(int)]
            for i0 in pos:
                ...
                for j, mc in enumerate(v42):                       # v4.2 carry tier + Q_j per window
                    cols[mc].append(m[11 + j])
            done += 1
    ...
    for k, v in cols.items():
        if k in ("split", "coarse", "subkind", "profile_id", "family", "noise_condition",
            ...
            arr[k] = pa.array(np.asarray(v))                        # gain_log10/quant_lsb/rail_mv floats
