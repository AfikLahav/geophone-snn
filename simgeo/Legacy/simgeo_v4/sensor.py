...
def coupling_response(n, fc, qc, fs=FS):
    """Ground-coupling resonance ahead of the geophone. Mode set by GEO_COUPLING_MODE.
...
def render_hp(v_signal, v_ground_noise, rng, profile_fc, profile_q, p_lines=0.5, sensor=None):
    ...
    # D5: ADS1015 hard rail on the recorded SUM (component blobs stay unclipped for exact SNR)
    out_mv = np.clip(clean_mv.astype(np.float64) + noise_mv.astype(np.float64),
                     -ADC_CLIP_MV, ADC_CLIP_MV)
    return out_mv.astype(np.float32), clean_mv, noise_mv, p
