"""Retrain with CDF-based detectability soft labels (replaces hard 0-dB cutoff + arbitrary
sigmoid(SNR/6)). Presence target per window = Pd_class(SNR) = a calibrated detectability CDF
fit to the measured Pd(SNR) curve at 1% Pfa. Ordinal (single/multiple) still hard, on old-present
windows, weighted by Pd. Saves model_cdf_ema.pt; evaluates real zero-shot 3-class vs the baseline run.
"""
...
LR, WARMUP, EMA_DECAY, DROPOUT, WD, CLIPZ, SCALE_REAL = 5e-4, 1000, 0.999, 0.1, 1e-4, 8.0, 25.4
# detectability CDF (fit to measured curve, 1% Pfa): Pd = Pfa + (1-Pfa)*Phi((snr-mu)/sig)
PFA = 0.01; MU = {"human": -16.5, "vehicle": -1.5, "animal": -10.5}; SIG = {"human": 14.1, "vehicle": 7.0, "animal": 11.7}
def pd_cdf(snr, cls): return (PFA + (1 - PFA) * norm.cdf((snr - MU[cls]) / SIG[cls])).astype(np.float32)
...
NF = len(FEATURES); fidx = [F.FEATURE_NAMES.index(f) for f in FEATURES]; LVL = {"none": 0, "single": 1, "multiple": 2}

cols = FEATURES + ["split", "human_level", "human_snr", "vehicle_level", "vehicle_snr", "animal_level", "animal_snr"]
...
json.dump({"mean": mu.tolist(), "std": sd.tolist(), "clip": CLIPZ, "features": FEATURES,
           "cdf": {"PFA": PFA, "MU": MU, "SIG": SIG}}, open(os.path.join(OUT, "scaler_cdf.json"), "w"))
def lvc(c): return df[c].map(LVL).fillna(df[c]).to_numpy().astype(int)
Hl, Vl, Al = lvc("human_level"), lvc("vehicle_level"), lvc("animal_level")
# CDF soft presence targets (clip snr to avoid -inf)
def clipsnr(c): return np.clip(df[c].to_numpy(float), -60, 60)
Hs, Vs, As = pd_cdf(clipsnr("human_snr"), "human"), pd_cdf(clipsnr("vehicle_snr"), "vehicle"), pd_cdf(clipsnr("animal_snr"), "animal")
print(f"CDF soft labels | human present>0.5 frac: {(Hs>0.5).mean():.3f} (was {(Hl>0).mean():.3f} hard) | "
      f"mean human soft {Hs.mean():.3f}")
...
def head_loss(lg, lvl, soft):
    l = Fnn.binary_cross_entropy_with_logits(lg[:, 0], soft)        # presence vs CDF soft target
    if lg.shape[1] == 2:
        m = lvl >= 1                                                # ordinal only on old-present (count known)
        if m.any(): l = l + Fnn.binary_cross_entropy_with_logits(lg[m, 1], (lvl[m] == 2).float())
    return l
...
def val_auroc(model):
    ...
    model.train(); a = []
    for nm in ph:
        s = torch.cat(ph[nm]).numpy(); yb = (L[nm][2] > 0).cpu().numpy()
        a.append(roc_auc_score(yb, s) if len(np.unique(yb)) == 2 else np.nan)
...
for step in range(MAX_STEPS):
    ...
    if (step + 1) % EVAL_EVERY == 0:
        au = val_auroc(ema)
        if (step + 1) % 2000 == 0: print(f"step {step+1} ema-AUROC {au:.4f} {(time.time()-t0)/60:.1f}m", flush=True)
...
torch.save(best_state, os.path.join(OUT, "model_cdf_ema.pt")); ema.load_state_dict(best_state)
print(f"DONE {(time.time()-t0)/60:.1f}m best ema-AUROC {best:.4f}")

# ---- real zero-shot 3-class (x25), per-head thresholds calibrated on synthetic val ----
from sklearn.metrics import accuracy_score, recall_score, balanced_accuracy_score
@torch.no_grad()
def probs(model, Xa):
    o = {"human": [], "vehicle": []}
    for j in range(0, len(Xa), 8192):
        functional.reset_net(model); r = model(to(Xa[j:j + 8192]))
...
out = {"thresholds": TH, "real_acc": float(accuracy_score(ry, pred)),
       ...
       "human_missed": int(((ry == "human") & (pred == "nothing")).sum()), "best_ema_val_auroc": float(best)}
json.dump(out, open(os.path.join(OUT, "CDF_RESULT.json"), "w"), indent=1)
print("\n=== CDF-labeled model, REAL zero-shot 3-class (x25) ===")
...
print("  baseline model (x25, 0-dB labels): acc 0.900 | human-rec 0.914 | human-missed 63")
print("wrote CDF_RESULT.json + model_cdf_ema.pt")
