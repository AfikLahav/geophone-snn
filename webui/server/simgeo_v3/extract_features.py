...
Split comes from simgeo_banks/splits_90_10.json (the 90/10 re-split), NOT the stale
...
def run_shard(args):
    ...
    has_wx = is_v2 and any(c[1] == "wind" for c in db.execute("PRAGMA table_info(scenes)"))
    if is_v2:
        wx = "s.wind,s.rain," if has_wx else ""        # v3 LEAN schema dropped wind/rain (metadata only)
        cur = db.execute("SELECT s.scene_id,s.profile_id,s.family,s.terrain_vs,s.coarse,s.subkind,"
                         f"s.split,{wx}w.clean_mv,w.noise_mv "
        ...
        try:
            if is_v2:
                if has_wx:
                    ...
                    sid, pid, fam, vs, coarse, sub, split, cm, nm = row; wind = rain = 0.0
                noise = np.frombuffer(nm, np.float32)
                x = (np.frombuffer(cm, np.float32) + noise) if cm is not None else noise
                # D5: ADS1015 hard rail where the model sees the signal — clip the recorded
                # clean+noise SUM at +-256 mV (no 0.125 mV quantize). Component blobs stay
                # unclipped in the DB so SNR labels remain exact; only the analyzed series is railed.
                x = np.clip(x.astype(np.float64), -256.0, 256.0).astype(np.float32)
