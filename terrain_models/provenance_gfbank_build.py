...
BANKS = os.path.join(HERE, "..", "simgeo_banks")
...
def stencil_for(profile):
    ...
    soft fine because its ghost arrives long after the window). Cure, two-sided:
      - contour damping alpha = 0.73*dk*Vp_max suppresses the ghost to 1%
        (alpha_min = -ln(0.01)*dk*c/(2*pi)), but is capped by re-amplification
        (exp(+alpha*t) noise growth): alpha <= 9/T (float64 headroom).
      - where the cap would bind, nk RISES instead (alpha cannot rescue an
        undersampled grid): nk >= kmax*0.73*Vp_max*T/9.
    ...
    nk = int(np.clip(max(kmax * 0.85, kmax * 0.73 * vp_max * T / 9.0),
                     8000, 26000))
    dk = kmax / nk
    alpha = float(min(0.73 * dk * vp_max, 9.0 / T))
    if 0.73 * dk * vp_max > 9.2 / T:
